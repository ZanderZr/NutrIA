# Ficha de Google Play — NutriControl
...
### Título (máx. 30)
```
NutriControl: Nutrición con IA
```
*(30/30 — justo en el límite. Alternativa más holgada: `NutriControl · Nutrición IA` = 27)*
...
Sin menús interminables ni buscar cada alimento en una base de datos. Hablas, y NutriControl lo entiende.
...
Introduce tu peso, altura y meta, y NutriControl calcula tus calorías y macros diarios. Además, ajusta tu objetivo de forma inteligente según tu peso real semana a semana.
...
▸ GRATIS
NutriControl usa tu propia clave gratuita de Google Gemini (se crea en 1 minuto, sin tarjeta). Sin suscripciones ni límites.
...
Aviso: NutriControl es una herramienta de apoyo. Las estimaciones son aproximadas y no sustituyen el consejo de un profesional de la salud.
```
...
No endless menus, no searching every food in a database. You speak, NutriControl understands.
...
Enter your weight, height and goal, and NutriControl works out your daily calories and macros. It even adjusts your target intelligently based on your real weight, week by week.
...
▸ FREE
NutriControl uses your own free Google Gemini key (created in 1 minute, no card required). No subscriptions, no limits.
...
Disclaimer: NutriControl is a support tool. Estimates are approximate and are not a substitute for professional medical advice.
```
...
keytool -genkey -v -keystore nutricontrol.keystore -alias nutricontrol \
